...
var spaceRegisterCmd = &cobra.Command{
	Use:   "register <email>",
...
func init() {
	...
	spaceCmd.AddCommand(spaceRegisterCmd)

	// Flags
	spaceCreateCmd.Flags().StringVar(&spaceFlags.description, "description", "", "Description for the space")
	spaceCreateCmd.Flags().BoolVar(&spaceFlags.json, "json", false, "Output in JSON format")

	spaceLsCmd.Flags().BoolVar(&spaceFlags.json, "json", false, "Output in JSON format")
	spaceUseCmd.Flags().BoolVar(&spaceFlags.json, "json", false, "Output in JSON format")
	spaceInfoCmd.Flags().BoolVar(&spaceFlags.json, "json", false, "Output in JSON format")

	spaceRegisterCmd.Flags().StringVar(&spaceFlags.spaceName, "space", "", "Space name or DID to register (defaults to current space)")
	spaceRegisterCmd.Flags().StringVarP(&spaceFlags.output, "output", "o", "", "Output path for delegation file")
}