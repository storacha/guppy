...
verify_email() {
	...
	while [ $attempt -lt $max_attempts ]; do
		...
		if [ -n "$message_id" ]; then
			echo "📬 Found email (ID: $message_id), retrieving verification link..."
			...
			echo "✅ Submitting approval form to: $verify_url"
			curl -sL -X POST "$verify_url" > /dev/null

			echo "✓ Email verified!"
			return 0
		else
			echo "⚠️ No verification link found in email, retrying..."
		fi
	...
	echo "❌ Failed to receive verification email after ${max_attempts} attempts"
	return 1
}
