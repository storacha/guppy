...
func TestAddShardsForUpload(t *testing.T) {
	t.Run("`space/blob/add`s, `space/blob/replicate`s, and `filecoin/offer`s a CAR for each shard", func(t *testing.T) {
		...
		err = shardsApi.AddNodeToUploadShards(t.Context(), upload.ID(), spaceDID, nodeCID1, data, func(shard *model.Shard) error { return nil })
		require.NoError(t, err)
		n, _, err = repo.FindOrCreateRawNode(t.Context(), nodeCID2, 1<<14, spaceDID, "some/other/path", source.ID(), 0)
		data = testutil.RandomBytes(t, int(n.Size()))
		require.NoError(t, err)
		err = shardsApi.AddNodeToUploadShards(t.Context(), upload.ID(), spaceDID, nodeCID2, data, func(shard *model.Shard) error { return nil })
		require.NoError(t, err)
		n, _, err = repo.FindOrCreateRawNode(t.Context(), nodeCID3, 1<<15, spaceDID, "yet/other/path", source.ID(), 0)
		require.NoError(t, err)
		data = testutil.RandomBytes(t, int(n.Size()))
		err = shardsApi.AddNodeToUploadShards(t.Context(), upload.ID(), spaceDID, nodeCID3, data, func(shard *model.Shard) error { return nil })
		require.NoError(t, err)
		...
		err = api.AddShardsForUpload(t.Context(), upload.ID(), spaceDID, func(shard *model.Shard) error { return nil })
		require.NoError(t, err)
		...
		err = shardsApi.CloseUploadShards(t.Context(), upload.ID(), func(shard *model.Shard) error { return nil })
		require.NoError(t, err)
		err = api.AddShardsForUpload(t.Context(), upload.ID(), spaceDID, func(shard *model.Shard) error { return nil })
		require.NoError(t, err)
