				...
				server.WithServiceMethod(
					...
					server.Provide(
						...
						func(
							...
							assert.Nil(t, *indexLink, "expected only one `upload/add` invocation")
...
func createUpload(t *testing.T, repo *sqlrepo.Repo, api preparation.API) *uploadsmodel.Upload {
	configuration, err := api.CreateConfiguration(t.Context(), "Large Upload Configuration", configurationsmodel.WithShardSize(1<<16))
	require.NoError(t, err)
...
func TestExecuteUpload(t *testing.T) {
	t.Run("uploads", func(t *testing.T) {
		...
		upload := createUpload(t, repo, api)
