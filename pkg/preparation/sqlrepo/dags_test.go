...
func TestDAGScan(t *testing.T) {
	t.Run("updates the DAG scan state and error message", func(t *testing.T) {
		...
		otherScans, err := repo.DAGScansForUploadByStatus(t.Context(), uploadID, model.DAGScanStateCompleted)
		require.NoError(t, err)
		...
		otherScans, err = repo.DAGScansForUploadByStatus(t.Context(), uploadID, model.DAGScanStatePending)
		require.NoError(t, err)
		require.Len(t, otherScans, 0)
	})
}
