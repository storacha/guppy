...
func (r *Repo) CreateSource(ctx context.Context, name string, path string, options ...sourcemodel.SourceOption) (*sourcemodel.Source, error) {
	...
	stmt, err := r.prepareStmt(ctx, `INSERT INTO sources (id, name, created_at, updated_at, kind, path, connection_params) VALUES (?, ?, ?, ?, ?, ?, ?)`)
	if err != nil {
...
func (r *Repo) GetSourceByID(ctx context.Context, sourceID id.SourceID) (*sourcemodel.Source, error) {
	stmt, err := r.prepareStmt(ctx, `SELECT id, name, created_at, updated_at, kind, path, connection_params FROM sources WHERE id = ?`)
	if err != nil {
...
func (r *Repo) GetSourceByName(ctx context.Context, name string) (*sourcemodel.Source, error) {
	stmt, err := r.prepareStmt(ctx, `SELECT id, name, created_at, updated_at, kind, path, connection_params FROM sources WHERE name = ?`)
	if err != nil {
...
func (r *Repo) getSourceByPath(ctx context.Context, path string) (*sourcemodel.Source, error) {
	stmt, err := r.prepareStmt(ctx, `SELECT id, name, created_at, updated_at, kind, path, connection_params FROM sources WHERE path = ?`)
	if err != nil {
...
func (r *Repo) UpdateSource(ctx context.Context, src *sourcemodel.Source) error {
	connectionParams := src.ConnectionParams()
	stmt, err := r.prepareStmt(ctx, `UPDATE sources SET name = ?, updated_at = ?, kind = ?, path = ?, connection_params = ? WHERE id = ?`)
	if err != nil {
