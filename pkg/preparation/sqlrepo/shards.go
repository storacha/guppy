...
func (r *repo) AddNodeToShard(ctx context.Context, shardID id.ShardID, nodeCID cid.Cid, offset uint64) error {
...
func (r *repo) ForEachNode(ctx context.Context, shardID id.ShardID, yield func(node dagsmodel.Node, shardOffset uint64) error) error {
	...
	for rows.Next() {
		...
		node, err := dagsmodel.ReadNodeFromDatabase(func(cid *cid.Cid, size *uint64, ufsdata *[]byte, path *string, sourceID *id.SourceID, offset *uint64) error {
			return rows.Scan(util.DbCid(cid), size, ufsdata, path, sourceID, offset, &shardOffset)
		})
