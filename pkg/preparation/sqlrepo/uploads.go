...
func (r *repo) CIDForFSEntry(ctx context.Context, fsEntryID types.FSEntryID) (cid.Cid, error) {
...
func (r *repo) newDAGScan(fsEntryID types.FSEntryID, isDirectory bool, uploadID types.UploadID) (dagmodel.DAGScan, error) {
	if isDirectory {
...
func (r *repo) CreateDAGScan(ctx context.Context, fsEntryID types.FSEntryID, isDirectory bool, uploadID types.UploadID) error {
	dagScan, err := r.newDAGScan(fsEntryID, isDirectory, uploadID)
	if err != nil {
		return err
	}

	return dagmodel.WriteDAGScanToDatabase(dagScan, func(kind string, fsEntryID types.FSEntryID, uploadID types.UploadID, createdAt time.Time, updatedAt time.Time, errorMessage *string, state dagmodel.DAGScanState, cid *cid.Cid) error {
		_, err := r.db.ExecContext(ctx,
