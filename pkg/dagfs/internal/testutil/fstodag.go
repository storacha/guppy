...
func BuildDAGFromFS(ctx context.Context, dagService ipld.DAGService, fsys fs.FS) (ipld.Node, error) {
	return buildDAGFromDir(ctx, dagService, fsys, ".")
}

// buildDAGFromDir recursively builds a DAG from a directory in the filesystem
func buildDAGFromDir(ctx context.Context, dagService ipld.DAGService, fsys fs.FS, dirPath string) (ipld.Node, error) {
	...
	for _, entry := range entries {
		...
		if entry.IsDir() {
			...
			childNode, err = buildDAGFromDir(ctx, dagService, fsys, entryPath)
			if err != nil {
				return nil, err
			}
		} else {
			// Build file node
			childNode, err = buildDAGFromFile(ctx, dagService, fsys, entryPath)
			if err != nil {
				return nil, err
			}
		}
...
func buildDAGFromFile(ctx context.Context, dagService ipld.DAGService, fsys fs.FS, filePath string) (ipld.Node, error) {
	// Open the file
	...
	// Build the DAG from the file
	return importer.BuildDagFromReader(dagService, spl)
}
